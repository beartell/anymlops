...
from anymlops.utils import check_cloud_credentials, load_yaml, timer
...
def force_destroy_configuration(config):
    ...
    with timer(logging, "destroying anymlops"):
        ...
        if "amazon_web_services" not in config:
            raise ValueError(
                "amazon_web_services section must exist in anymlops-config.yaml"
            )
        ...
        if region == "":
            raise ValueError(
                "amazon_web_services.region must exist in anymlops-config.yaml"
            )
