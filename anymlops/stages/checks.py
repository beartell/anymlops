...
def stage_02_infrastructure(stage_outputs, anymlops_config):
...
def stage_03_kubernetes_initialize(stage_outputs, anymlops_config):
    from kubernetes import client, config
    from kubernetes.client.rest import ApiException
    ...
    if anymlops_config["namespace"] not in namespaces:
        print(
...
def stage_04_kubernetes_ingress(stage_outputs, anymlops_config):
...
def stage_06_kubernetes_keycloak_configuration(stage_outputs, config):
    ...
    def _attempt_keycloak_connection(
        ...
        for i in range(num_attempts):
            try:
                ...
                if anymlops_realm in existing_realms:
                    print(
                        f"Attempt {i+1} succeeded connecting to keycloak and anymlops realm={anymlops_realm} exists"
                    )
                    return True
                else:
                    print(
                        f"Attempt {i+1} succeeded connecting to keycloak but anymlops realm did not exist"
                    )
            except KeycloakError:
                print(f"Attempt {i+1} failed connecting to keycloak master realm")
            time.sleep(timeout)
    ...
    if not _attempt_keycloak_connection(
        ...
        stage_outputs[directory]["keycloak_credentials"]["value"]["realm"],
        stage_outputs[directory]["keycloak_credentials"]["value"]["client_id"],
        anymlops_realm=stage_outputs["stages/06-kubernetes-keycloak-configuration"][
            "realm_id"
        ]["value"],
        verify=False,
    ):
        print(
            "ERROR: unable to connect to keycloak master realm and ensure that anymlops realm exists"
        )
        sys.exit(1)

    print("Keycloak service successfully started with anymlops realm")
