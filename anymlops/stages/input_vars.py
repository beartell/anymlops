...
from anymlops.constants import DEFAULT_CONDA_STORE_IMAGE_TAG, DEFAULT_TRAEFIK_IMAGE_TAG
    ...
    if config["provider"] == "local":
        return {
            "kubeconfig_filename": os.path.join(
                tempfile.gettempdir(), "ANYMLOPS_KUBECONFIG"
            ),
...
def stage_07_kubernetes_services(stage_outputs, config):
    ...
    if config["theme"]["jupyterhub"].get("display_version") and (
        ...
        jupyterhub_theme.update({"version": f"v{config['anymlops_version']}"})
    ...
    return {
        ...
        "conda-store-default-namespace": config.get("conda_store", {}).get(
            "default_namespace", "anymlops-git"
        ),
...
def stage_08_anymlops_tf_extensions(stage_outputs, config):
    return {
        "environment": config["namespace"],
        "endpoint": config["domain"],
        "realm_id": stage_outputs["stages/06-kubernetes-keycloak-configuration"][
        ...
        "anymlops_config_yaml": config,
        "keycloak_anymlops_bot_password": stage_outputs["stages/05-kubernetes-keycloak"][
            "keycloak_anymlops_bot_password"
        ]["value"],
