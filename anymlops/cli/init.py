...
from anymlops.initialize import render_config
...
def handle_init(inputs: InitInputs):
    ...
    config = render_config(
        ...
        anymlops_domain=inputs.domain_name,
    ...
    try:
        with open("anymlops-config.yaml", "x") as f:
            yaml.dump(config, f)
    except FileExistsError:
        raise ValueError(
            "A anymlops-config.yaml file already exists. Please move or delete it and try again."
        )
    ...
    try:
        rich.print(
            (
                ...
                "You will be asked a few questions to generate your [purple]anymlops-config.yaml[/purple]. "
        ...
        if inputs.auth_provider.lower() == AuthenticationEnum.auth0.value.lower():
            ...
            rich.print(
                (
                    ...
                    f"The `Authorization callback URL` is set to: [green]https://{inputs.domain_name}/auth/realms/anymlops/broker/github/endpoint[/green]\n\n"
                )
            )
        ...
        if questionary.confirm(
            ...
            if git_provider == GitRepoEnum.github.value.lower():
                inputs.repository_auto_provision = questionary.confirm(
                    f"Would you like anymlops to create a remote repository on {git_provider}?",
                    default=False,
                    qmark=qmark,
                ).unsafe_ask()
        ...
        rich.print(
            (
                "\n\n\t:sparkles: [bold]Congratulations[/bold], you have generated the all important [purple]anymlops-config.yaml[/purple] file :sparkles:\n\n"
                "You can always make changes to your [purple]anymlops-config.yaml[/purple] file by editing the file directly.\n"
                "If you do make changes to it you can ensure it's still a valid configuration by running:\n\n"
                "\t[green]anymlops validate --config path/to/anymlops-config.yaml[/green]\n\n"
            )
        )

        base_cmd = f"anymlops init {inputs.cloud_provider}"
        ...
        rich.print(
            (
                "For reference, if the previous Guided Init answers were converted into a direct [green]anymlops init[/green] command, it would be:\n\n"
