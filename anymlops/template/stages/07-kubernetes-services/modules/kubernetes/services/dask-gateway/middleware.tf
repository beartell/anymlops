resource "kubernetes_manifest" "gateway-middleware" {
  manifest = {
    ...
    kind       = "Middleware"
    metadata = {
      name      = "anymlops-dask-gateway-gateway-api"
...
resource "kubernetes_manifest" "chain-middleware" {
  manifest = {
    ...
    kind       = "Middleware"
    metadata = {
      name      = "anymlops-dask-gateway-chain" # Updated name to -chain from -cluster to avoid upgrade confusion
      namespace = var.namespace
    }
    spec = {
      chain = {
...
resource "kubernetes_manifest" "cluster-middleware-stripprefix" {
  manifest = {
    ...
    kind       = "Middleware"
    metadata = {
      name      = "anymlops-dask-gateway-cluster-stripprefix"
      namespace = var.namespace
    }
    spec = {
      stripPrefixRegex = {
