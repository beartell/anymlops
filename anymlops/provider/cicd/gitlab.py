...
def gen_gitlab_ci(config):
    ...
    pip_install = pip_install_anymlops(config["anymlops_version"])

    render_vars = {
        "COMMIT_MSG": "anymlops-config.yaml automated commit: {{ '$CI_COMMIT_SHA' }}",
    }

    script = [
        f"git checkout {branch}",
        f"{pip_install}",
        "anymlops deploy --config anymlops-config.yaml --disable-prompt --skip-remote-state-provision",
    ]
    ...
    rules = [
        GLCI_rules(
            ...
            changes=["anymlops-config.yaml"],
        )
    ]

    render_anymlops = GLCI_job(
    ...
    return GLCI(
        __root__={
            "render-anymlops": render_anymlops,
        }
    )
