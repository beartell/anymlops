...
from anymlops.constants import LATEST_SUPPORTED_PYTHON_VERSION
from anymlops.provider.cicd.common import pip_install_anymlops
...
def gen_anymlops_ops(config):
    ...
    anymlops_version = config["anymlops_version"]

    push = GHA_on_extras(branches=[branch], paths=["anymlops-config.yaml"])
    on = GHA_on(__root__={"push": push})

    step1 = checkout_image_step()
    step2 = setup_python_step()
    step3 = install_anymlops_step(anymlops_version)
    ...
    step5 = GHA_job_step(
        name="Push Changes",
        run=(
            "git config user.email 'anymlops@quansight.com' ; "
        ...
        env={
            "COMMIT_MSG": GHA_job_steps_extras(
                __root__="anymlops-config.yaml automated commit: ${{ github.sha }}"
            )
        },
    )
    if commit_render:
...
def gen_anymlops_linter(config):
    ...
    pull_request = GHA_on_extras(branches=[branch], paths=["anymlops-config.yaml"])
    ...
    job1 = GHA_job_id(
        name="anymlops", runs_on_="ubuntu-latest", steps=[step1, step2, step3, step4]
    )
    jobs = GHA_jobs(
        __root__={
            "anymlops-validate": job1,
        }
    )
